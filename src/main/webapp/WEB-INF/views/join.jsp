...
<body>
	...
	<div id="body">
		<form action="join" method="post" name="f1" enctype="multipart/form-data">
			<label for="">ID</label><input type="text" name="userid" /><a href="" id="checkID">중복확인</a> <span class="check"> 사용가능한 아이디입니다.</span><br>
			<label for="">NAME</label><input type="text" name="username" /><br>${userVO.userid }
			<label for="">PW</label><input type="text" name="userpw0" /><br>
			<label for="">PW CONFIRM</label><input type="text" name="userpw" /> <span class="check"> 비밀번호를 확인하세요.</span><br>
...
$("form[name='f1']").submit(function() {
	if($("input[name='userpw0']").val() != $("input[name='userpw']").val()){
		var $next = $("input[name='userpw']").nextAll(".check").eq(0)
